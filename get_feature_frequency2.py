...
cwd = os.getcwd()
smallfile_list = glob.glob(os.path.join(cwd,'corpus_split','small_file_*.txt'))
count = 0
for smallfile in smallfile_list:
    ...
    f.close()
    with open(os.path.join(cwd,'corpus_split','corpus_split_out','small_file_'+str(count)+'.csv'), 'w') as outfile:
